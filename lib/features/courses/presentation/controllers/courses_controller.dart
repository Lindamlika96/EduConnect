...
class CoursesController extends ChangeNotifier {
  ...
  Future<void> loadAll({String? query, int? level, int? language}) async {
    ...
    try {
      final rows =
      await _dao.fetchCourses(q: query, level: level, language: language);
  ...
  Future<void> fetchMyCoursesCompleted(int userId) async {
    ...
    try {
      ...
      state.myCompleted = rows
        ...
        progressPercent:
        (m['progress_percent'] as num?)?.toDouble() ?? 0.0,
  ...
  Future<void> updateProgressMax(
      int userId, int courseId, double percent) async {
    final now = DateTime.now().millisecondsSinceEpoch;
  ...
  // ---------- Résumé (Gemini) ----------
  Future<CourseSummary> summarizeCourse({
    required int courseId,
    required String language,
    required String assetPdfPath, // non-null si asset
    bool forceRefresh = false,
  }) async {
    final useCase = _di.getCourseSummaryUseCase;
    return useCase(
      courseId: courseId,
      language: language,
  ...
  // Compat
  Future<List<Map<String, Object?>>> listAll({String? q}) =>
      _dao.fetchCourses(q: q);
